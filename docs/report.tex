...
One of the features that has changed multiple times over the course of 
developing the lab is the \texttt{apiFetch()} utility for REST services on 
frontend. The initial versions was a slim wrapper around \texttt{fetch()} 
that accepted path and params and called the network. In version 2, query 
parameter concatenation was added because I needed it for event searching 
and filtering (\href{https://github.com/ramz1t/hkr-fullstack-lab/commit/14deea1964b07d65fcfe9b46de8608b03753afe5}{14deea1}). 
Later, when I decided to deploy the project, it became clear that 
authentication with user scope needed to be implemented to avoid data 
inconsistencies. For that I used JWT authentication, so the \texttt{Authorization} 
header had to be introduced in the utility 
(\href{https://github.com/ramz1t/hkr-fullstack-lab/commit/5dc0a605117a689ca0ac7a00e3c0045e82617651}{5dc0a60}).
